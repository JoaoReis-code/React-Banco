...
export default function Deposito(){
    ...
    function depositar(){
        if(valor > 0){
            contaLogada.deposito(parseFloat(valor).toFixed(2))
            Alert.alert("Valor adicionado com sucesso")
            setValor(0)
        }else{
            Alert.alert("Voce nao pode depositar um valor negativo")
            setValor(0)
        }        
    ...
    return(
        ...
        <Animatable.View animation="fadeInUp" delay={400} style={styles.containerForm}>
                <Text style={styles.title}>Deposito</Text>
                <TextInput
                    style={styles.input}
                    keyboardType="numeric"
                    onChangeText={setValor}
                    value={valor}
                    placeholder="Informe quanto dejesa depositar"
                />
